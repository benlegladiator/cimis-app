    ...
    if (!function_exists('getallheaders')) {
        ...
        foreach ($_SERVER as $name => $value) {
            if (substr($name, 0, 5) == 'HTTP_') {
                // Convertir HTTP_X_API_KEY en X-API-KEY
                $header_name = str_replace(' ', '-', ucwords(strtolower(str_replace('_', ' ', substr($name, 5)))));
                $headers[$header_name] = $value;
            }
            // Aussi vérifier les headers directement (pour certains serveurs)
            elseif ($name == 'X_API_KEY') {
...
function verifyApiKey() {
    ...
    error_log("Clé API reçue: " . ($api_key ? 'OUI' : 'NON'));
    error_log("Clé API attendue: " . SIADOC_API_KEY);
    ...
    if (!$api_key || $api_key !== SIADOC_API_KEY) {
        ...
        echo json_encode([
            ...
            'debug' => [
                ...
                'expected_key' => SIADOC_API_KEY,
                'headers' => $headers,
                'get_params' => $_GET,
                'server_vars' => array_filter($_SERVER, function($key) {
                    return strpos($key, 'HTTP_') === 0 || strpos($key, 'X_') === 0 || strpos($key, 'CONTENT_') === 0;
    ...
    case 'help':
        sendResponse([
            ...
            'champs_disponibles' => [
                ...
                'statut_carte' => 'Statut de la carte (ACTIVE/EXPIREE/SUSPENDUE/PERDUE/RENOUVELEE)',
                'date_modification' => 'Date de dernière modification',
                'supprimer' => 'Indicateur de suppression (1=actif, 0=supprimé)'
            ],
            'exemples_requetes' => [
                ...
                'Militaires actifs âgés de 25-35 ans' => '/cartes?statut_carte=ACTIVE&age_min=25&age_max=35',
                'Femmes dans l\'Armée de l\'Air' => '/cartes?sexe=FEMININ&unite=ARMÉE DE L\'AIR'
            ]
        ]);
        break;
