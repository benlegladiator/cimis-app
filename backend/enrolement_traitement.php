...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if (!isset($_POST['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) {
            throw new Exception("Erreur de sécurité CSRF : tentative d'envoi non autorisée.");
        }
        ...
        $annee_galon = !empty($date_promotion) ? date('Y', strtotime($date_promotion)) : null;
        ...
        if (!empty($_POST['photo_data']) && $_POST['photo_data'] !== '') {
            ...
            if (preg_match('/^data:image\/(\w+);base64,/', $photoData, $matches)) {
                $mime_extension = strtolower($matches[1]);
                if ($mime_extension === 'jpeg' || $mime_extension === 'jpg') {
                    $photoExtension = 'jpg';
                } elseif ($mime_extension === 'png') {
                    $photoExtension = 'png';
                } else {
                    throw new Exception("Format de photo non autorisé via base64. Utilisez JPEG ou PNG.");
                }
                // Supprimer le préfixe base64
                $photoData = preg_replace('/^data:image\/(\w+);base64,/', '', $photoData);
            } else {
                throw new Exception("Format de données photo invalide.");
            }
        }
        ...
        elseif (isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
            ...
            // Validation magique du type MIME
            ...
            $photoExtension = $allowedTypes[$mime_type];
        ...
        $date_dernier_grade = !empty($annee_galon) ? $annee_galon . '-01-01' : null; // 1er janvier de l'année
