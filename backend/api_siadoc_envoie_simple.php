<?php
/**
 * API SIADOC ENVOIE - VERSION SIMPLE
 * Envoie seulement les données importantes du militaire
 * Sans les colonnes inutiles (statut_carte, supprimer, suspendu, etc.)
 */
...
switch ($action) {
    case 'help':
        sendResponse([
            ...
            'note' => 'API simplifiée - seulement les données importantes'
        ]);
        break;
    ...
    case 'carte':
        ...
        try {
            $stmt = $pdo->prepare("
                ...
                AND (c.suspendus = 0 OR c.supprimer = 1)
                LIMIT 1
            ");
