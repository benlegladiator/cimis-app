...
header('Access-Control-Max-Age: 86400'); // 24 heures de cache CORS
...
function encodeImageToBase64($image_path) {
    ...
    // Essayer plusieurs chemins possibles pour InfinityFree
    $paths_to_try = [
        $_SERVER['DOCUMENT_ROOT'] . '/' . $image_path,
        $_SERVER['DOCUMENT_ROOT'] . '/cimcim/' . $image_path,
        $_SERVER['DOCUMENT_ROOT'] . '/htdocs/' . $image_path,
        $image_path
    ];
...
switch ($action) {
    ...
    case 'debug':
        // Endpoint de débogage pour vérifier les chemins des images
        try {
            $stmt = $pdo->prepare("SELECT matricule, photo, code_qr FROM candidat WHERE supprimer = 1 LIMIT 5");
            $stmt->execute();
