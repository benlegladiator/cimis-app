...
function getGradeImage($grade) {
    ...
    $grade_images = [
        ...
        'general d armee (ga)' => 'img/galons/general_armé.png',
    ...
    $grade_normalise = str_replace(['é', 'è', 'ê', 'ë'], 'e', $grade_normalise);
    $grade_normalise = str_replace(['à', 'â', 'ä'], 'a', $grade_normalise);
    $grade_normalise = str_replace(['î', 'ï'], 'i', $grade_normalise);
    $grade_normalise = str_replace(['ô', 'ö'], 'o', $grade_normalise);
    $grade_normalise = str_replace(['ù', 'û', 'ü'], 'u', $grade_normalise);
...
function formatGradeDisplay($grade) {
    ...
    // Supprimer les accents mais garder les tirets
    $grade_normalise = str_replace(['É', 'È', 'Ê', 'Ë'], 'E', $grade_normalise);
    $grade_normalise = str_replace(['À', 'Â', 'Ä'], 'A', $grade_normalise);
    $grade_normalise = str_replace(['Î', 'Ï'], 'I', $grade_normalise);
    $grade_normalise = str_replace(['Ô', 'Ö'], 'O', $grade_normalise);
