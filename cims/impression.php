...
if (isset($_POST['action']) && $_POST['action'] == 'delete_multiple') {
    try {
        ...
        if (!empty($ids) && is_array($ids)) {
            ...
            $sql = "UPDATE candidat SET supprimer = 0, supprimer_par = :username, date_suppression = NOW() WHERE id IN ($placeholders)";
            $stmt = $pdo->prepare($sql);
            $stmt->execute(array_merge($ids, ['username' => $_SESSION['username']]));
...
if (isset($_POST['ajax_search'])) {
    ...
    $sql .= " ORDER BY date_dernier_grade DESC LIMIT 100";
        ...
        foreach ($personnels as $personnel) {
            echo '<div class="personnel-item' . ($personnel['suspendus'] == 1 ? ' suspended' : '') . '">
                    <div class="personnel-checkbox-wrapper">
                        <input type="checkbox" name="selected_personnels[]" value="' . htmlspecialchars($personnel['matricule']) . '" 
            ...
            if (!empty($personnel['photo']) && file_exists($personnel['photo'])) {
                echo '<img src="' . $personnel['photo'] . '" alt="Photo">';
            } else {
                echo '<i class="fa-solid fa-user" style="font-size: 2rem; color: var(--text-muted);"></i>';
            }
            echo '</div>
                    ...
                    <div class="personnel-actions">
                        ...
                        <a href="impression_carte_v2.php?matricule=' . urlencode($personnel['matricule']) . '&mode=recto-verso" 
    ...
    // Afficher les actions batch si des personnels sont trouvés
    if (!empty($personnels)) {
        echo '<div class="batch-actions" style="margin-top: 2rem; padding-top: 1.5rem; border-top: 2px solid rgba(74, 222, 128, 0.3);">
                <button class="btn" onclick="selectAll()">
                    <i class="fa-solid fa-check-square"></i> SÉLECTIONNER TOUT
                </button>
                <button class="btn btn-primary" onclick="visualizeMultiple()">
                    <i class="fa-solid fa-eye"></i> VISUALISER SÉLECTION
                </button>
                <button class="btn" onclick="generateBatch()">
                    <i class="fa-solid fa-file-pdf"></i> GÉNÉRER PDF MULTIPLE
                </button>
                <button class="btn btn-logout" onclick="deleteSelected()" style="padding: 2px;">
                    <i class="fa-solid fa-trash"></i> suprimer sélection
                </button>
              </div>';
    }
...
if (isset($_POST['export'])) {
    ...
    $sql .= " ORDER BY date_dernier_grade DESC";
            ...
            <div class="container">
                ...
                <div class="search-section">
                    ...
                    <div class="search-actions">
                        <button type="button" class="btn" onclick="clearSearch()" style="padding: 2px;">
                            <i class="fa-solid fa-times"></i> suprimer / Clear
                        </button>
                        <button type="button" class="btn btn-primary" onclick="exportResults()">
                            <i class="fa-solid fa-download"></i> <span style="color: black;">EXPORTER PDF / EXPORT PDF</span>
                        </button>
                        <button type="button" class="btn btn-secondary" onclick="printSelected()">
                            <i class="fa-solid fa-print"></i> IMPRIMER SÉLECTION / PRINT SELECTION
                        </button>
                    </div>
                </div>
                            ...
                            <?php foreach ($personnels as $personnel): ?>
                                <div class="personnel-item<?php echo $personnel['suspendus'] == 1 ? ' suspended' : ''; ?>">
                                    ...
                                    <div class="personnel-photo">
                                        <?php if (!empty($personnel['photo']) && file_exists($personnel['photo'])): ?>
                                            <img src="<?php echo $personnel['photo']; ?>" alt="Photo">
                                        <?php else: ?>
                                            <i class="fa-solid fa-user" style="font-size: 2rem; color: var(--text-muted);"></i>
                                        <?php endif; ?>
                                    </div>
                                    <div class="personnel-info">
                                    ...
                                    <div class="personnel-actions">
                                        ...
                                        <a href="impression_carte_v2.php?matricule=<?php echo urlencode($personnel['matricule']); ?>&mode=recto-verso" 
                                           class="btn btn-sm" style="background: linear-gradient(135deg, #007bff, #0056b3); color: white;" title="Impression PVC Optimisée (85.60×53.98mm - 0 marge)">
                                            <i class="fa-solid fa-credit-card"></i> <span style="font-size: 10px;">PVC</span>
                                        </a>
    ...
    <script>
        ...
        function deleteSelected() {
            ...
            if (confirm(`Êtes-vous sûr de vouloir déplacer les ${selected.length} carte(s) sélectionnée(s) dans la corbeille ?\n\nAre you sure you want to move the ${selected.length} selected card(s) to trash?`)) {
                // Récupérer les IDs des candidats sélectionnés
                const ids = [];
