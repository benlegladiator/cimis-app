...
if (isset($_POST['action']) && $_POST['action'] == 'restore') {
    try {
        ...
        if (!isset($_POST['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) {
            throw new Exception("Erreur de sécurité CSRF : jeton de sécurité invalide.");
        }
            ...
            if (!empty($ids)) {
                ...
                try {
                    ...
                    foreach ($restored_cands as $rc) {
                        ...
                        if (!empty($m)) {
                            notifierSiadocStatutCarte($m, 'ACTIVE', 'Restauration depuis la corbeille');
                        }
                    }
                } catch (Exception $ex_siadoc) {}

                // Restaurer les cartes
                $sql = "UPDATE candidat SET supprimer = 1, supprimer_par = NULL, date_suppression = NULL WHERE id IN ($placeholders) AND supprimer_par = ?";
                $stmt = $pdo->prepare($sql);
                $stmt->execute(array_merge($ids, [$username]));
                
                $_SESSION['success'] = count($ids) . " carte(s) restaurée(s) avec succès / " . count($ids) . " card(s) restored successfully";
            } else {
                $_SESSION['error'] = "Aucune carte valide sélectionnée / No valid card selected";
            }
        } else {
