...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_submit'])) {
    try {
        ...
        if (!isset($_POST['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) {
            throw new Exception("Erreur de sécurité CSRF : tentative de modification non autorisée.");
        }
        ...
        if (isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
            ...
            $extension = $mime_map[$mime_type] ?? 'jpg';
...
<body>
    ...
    <script>
        ...
        function updateGrades() {
            ...
            if (gradesParUnite && gradesParUnite[unite]) {
                ...
                gradesParUnite[unite].forEach(function(grade) {
                    ...
                    option.value = grade;
                    option.textContent = grade;
                    // Comparaison normalisée pour gérer les différences d'encodage
                    const gradeNormalized = grade.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().trim();
                    const currentGradeNormalized = currentGrade.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().trim();
                    if (gradeNormalized === currentGradeNormalized) {
                        option.selected = true;
                        console.log('Grade présélectionné (normalisé):', grade, '->', currentGrade);
                    }
                    gradeSelect.appendChild(option);
                });
            } else {
        ...
        function submitForm() {
            ...
            if (!submitBtn) {
                console.error('Bouton submit non trouvé - essai avec sélecteurs multiples');
                // Debug: afficher tous les boutons trouvés
                console.log('Boutons trouvés:', document.querySelectorAll('button'));
                console.log('Boutons avec type submit:', document.querySelectorAll('button[type="submit"]'));
                return;
            }
