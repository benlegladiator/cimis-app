...
// Vérification de l'authentification pour les endpoints JSON
if (!($_SERVER['REQUEST_METHOD'] === 'GET' && !isset($_GET['action']))) {
    $headers = function_exists('getallheaders') ? getallheaders() : [];
    $apiKey = $headers['X-API-KEY'] ?? $headers['x-api-key'] ?? $_SERVER['HTTP_X_API_KEY'] ?? $_GET['api_key'] ?? null;
    
    $is_authenticated = (isset($_SESSION['user_logged_in']) && $_SESSION['user_logged_in'] === true) || 
                        ($apiKey === SIADOC_API_KEY);
                        
    if (!$is_authenticated) {
        http_response_code(401);
        echo json_encode([
            'error' => 'Non autorisé. Veuillez fournir une clé API valide ou vous connecter.',
            'timestamp' => date('c')
        ]);
        exit();
    }
